...
class semContextBlock(nn.Module):
	"""
	...
	def forward(self, x):
		...
		# This is the image pooling (gloabl avg pooling + conv layers) # From the paper  deeplab_v3 not the +
		...
		x = torch.cat((x1, x2, x3, x4, x5), 1) # Concatenate all different features 4 Atrous Convs + 1 Image Pooling
